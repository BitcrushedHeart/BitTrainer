...
class BinaryTask(TrainingTask):
    ...
    def make_context(
        ...
        checkpoint_dir = (
            ...
            else concept_folder / "checkpoints"
        )
    ...
    def prepare_data(self, ctx: TaskContext) -> None:
        ...
        if config.use_cache:
            ...
            cache_root = (
                ...
                if config.cache_dir
                else (concept_folder / ".smart_cache")
            )
        ...
        if config.sourceless:
            ...
            train_ds = ConceptDataset(
                ...
                negative_paths=config.train_negative_paths,
                hard_negative_paths=(
                    config.train_hard_negative_paths
                    if config.train_hard_negative_paths is not None
            ...
            val_ds = ConceptDataset(
                ...
                negative_paths=config.val_negative_paths,
                hard_negative_paths=(
                    config.val_hard_negative_paths
                    if config.val_hard_negative_paths is not None
                    else config.hard_negative_paths
                ),
    ...
    def create_model(self, ctx: TaskContext, resume_state: dict | None):
        ...
        if resume_state is not None:
            ...
            try:
                ...
                logger.warning(
                    "Failed to load existing checkpoint, starting from pretrained",
                    exc_info=True,
                )
                model = bt._fresh_binary_model(config, dtype=dtype).to(device)
        else:
    ...
    def resumed_message(
        ...
        return {
            ...
            "resumed_from": str(self.config.resume_from),
            "epoch": start_epoch,
    ...
    def build_loaders(
        ...
        if self._loader_kwargs is None:
            self._loader_kwargs = loader_kwargs(
                config.dataloader_workers, prefetch_factor=3
            )
    ...
    def validate(self, ctx: TaskContext, model, epoch: int, train_result) -> dict:
        ...
        val_result = bt.evaluate(
            eval_model, self._val_loader, self.criterion, ctx.device, ctx.dtype
        )
